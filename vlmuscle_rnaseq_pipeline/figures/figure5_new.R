#### Figure 4 --- DE analysis of different normalization methods TIME EFFECTS #####
...
week2_naive <- volcano_data %>%
  ...
  # Annotate plot
  annotate("text", x = -2.5, y = 78, label = "Naïve", hjust = 0, size = 2.2) +
...
week2_libsize <- volcano_data %>%
  ...
  annotate("text", x = -2.5, y = 78, label = "Effective library-\nsize", hjust = 0,vjust = 1, size = 2.2) +
...
week2_tissue <- volcano_data %>%
  ...
  annotate("text", x = -2.5, y = 78, label = "Tissue offset", hjust = 0, size = 2.2) +
...
upset.list.w2 <- modified_upset(list.up = venn_list_up[1:3], 
                         ...
                         line.size = 0.5, # size of lines in figures (may be changed later)
...
week12_naive <- volcano_data %>%
  ...
  annotate("text", x = -2.5, y = 39, label = "Naïve", hjust = 0, size = 2.2) +
...
week12_tissue <- volcano_data %>%
  ...
  annotate("text", x = -2.5, y = 39, label = "Tissue-offset", hjust = 0, size = 2.2) +
...
ggsave("figures/figure4.pdf", plot = figure4, width = 8.9, height = 18, 
       dpi = 600,
